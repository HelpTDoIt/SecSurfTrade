...
class TestVWAPRules:
    ...
    def test_sell_below_vwap(self):
        ...
        quote = _quote(
            "SPY", bid=99.50, ask=99.55, last=99.52, prev_close=100.00, volume=200_000
        )
...
class TestBuyUrgencyEscalation:
    def test_no_escalation_before_90min(self):
        quote = _quote("SPY", bid=100.0, ask=100.02, last=100.01, prev_close=100.0)
    ...
    def test_no_thin_for_liquid_ticker(self):
        ...
        watchlist = {
            "SPY": WatchlistRow(
                ...
                volume=50_000_000,
                prev_close=500.0,
                avg_vol_10d=100_000_000,
                avg_vol_90d=100_000_000,
                div_ex_date="",
                div_local=0.0,
                vwap=500.0,
                ts=datetime.now(tz=timezone.utc),
...
class TestBuyProgress:
    def test_time_elapsed_pct(self):
        ...
        assert _time_elapsed_pct(datetime(2026, 5, 6, 12, 45)) == pytest.approx(
            50.0, abs=0.5
        )
        # Market close: 100%

...
j.write("requote_confirmed", {
    "original_chunk": "s2",
    "new_chunk": "s2b",
    "new_limit": sugg.new_limit,
    "remaining_qty": sugg.remaining_qty,
})
print(f"       Confirmed: new chunk s2b — {sugg.remaining_qty:.0f} sh @ ${sugg.new_limit:.4f}")
...
if _all_sells_terminal("Roth IRA", order_map, ["s1", "s2"]):
    proceeds = _actual_proceeds("Roth IRA", order_map, ["s1", "s2"])
    j.write("recompute_trigger", {"account": "Roth IRA", "actual_proceeds": proceeds})
    print(f"       All sells complete — recompute triggered. Proceeds: ${proceeds:,.2f}")
...
for line in jpath.read_text(encoding="utf-8").splitlines():
    ...
    print(f"  [{entry['ts'][11:19]}] {entry['event_type']:25s} {json.dumps(entry['payload'])}")

...
Account names must match the **Account Name** field in your Fidelity CSV headers (matching is case-insensitive).
...
TRADING MORNING (pre-market):
  3. Download 3 Fidelity CSVs to Downloads folder
  4. Run cli.compute → state.json  (auto-detects CSVs)
  5. Open React calculator → Import State → review Trades tab
  (Alternatively: load CSVs directly in Setup tab, enter signals, Calculate)
...
The mapping tolerates year prefix changes (YE25 → YE26) and minor whitespace differences automatically. Update `STRATEGY_MAP` in `scripts/sectorsurfer_signals.py` only if SectorSurfer changes the base strategy name.
...
| 144 tests fail / unexpected failures | Dependencies out of sync | `pip install -e "fidelity_rebalancer/[dev]"` and re-run |
